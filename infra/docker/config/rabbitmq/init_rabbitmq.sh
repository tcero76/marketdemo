...
until rabbitmqadmin "${RABBITMQ_ARGS[@]}" list queues >/dev/null 2>&1
do
...
create_queue_if_missing() {
  ...
  exists=$(rabbitmqadmin "${RABBITMQ_ARGS[@]}" --vhost="$vhost" list queues name | grep -w "$queue" || true)

  if [ -z "$exists" ]; then
    echo "Creando cola $queue en vhost $vhost"
    rabbitmqadmin "${RABBITMQ_ARGS[@]}" --vhost="$vhost" declare queue \
...
create_exchange_if_missing() {
  ...
  exists=$(rabbitmqadmin "${RABBITMQ_ARGS[@]}" --vhost="$vhost" list exchanges name | grep -w "$exchange" || true)

  if [ -z "$exists" ]; then
    echo "Creando exchange $exchange (type: $type) en vhost $vhost"
    rabbitmqadmin "${RABBITMQ_ARGS[@]}" --vhost="$vhost" declare exchange \
...
echo "========================================"
echo "Inicialización de RabbitMQ completada."
echo "========================================"